...
namespace Printer.Controllers
{
    ...
    public class Controller : ControllerBase
    {
        ...
        public async Task<IActionResult> PrintPDFAsync(IFormFile file, string tpsIP, int port = 9100, float zoomImage = 1, bool inversion = false)
        {
            ...
            // Создаем временный путь для сохранения PDF файла
            string tempPath = Path.Combine(Path.GetTempPath(), file.FileName);

            try
            {
                // Сохраняем загруженный файл в временную папку
                ...
                renderPdfToFile(tempPath, "outputImage.png", 300); // Укажите имя выходного файла по вашему усмотрению
            }
            catch (Exception ex)
            {
            ...
            return Ok("PDF успешно обработан");
        }
                ...
                using (var doc = PdfDocument.Load(pdfFilename))
                {
                    ...
                    for (int page = 0; page < doc.PageCount; page++)
                    {
                        using (var img = doc.Render(page, dpi, dpi, false))
                        {
                            ...
                            string outputFilePath = Path.Combine(Path.GetTempPath(), $"page_{page}_{outputImageFilename}");
